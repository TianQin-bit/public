...
{
    struct observ ref={615553864.184,0,0.398497821984698,-2.92342649709050};
    double k1,k2,k3,k4;
    int pf=0,namepf=0,steplimit=1000000,degtran_0[3],degtran_1[3],degtran_2[3];
    FILE *outf;
    ...
    for(pf=0;pf<steplimit;pf++)
    {
        ...
        alpha=(alpha>-pi)?(alpha):(alpha+2*pi);
    ...
    degtran_0[0]=(int) (theta*180/pi);degtran_0[1]=(int) 60*(theta*180/pi-degtran_0[0]);degtran_0[2]=(int) 3600*(theta*180/pi-degtran_0[0]-degtran_0[1]);
    degtran_1[0]=(int) (alpha*180/pi);degtran_1[1]=(int) 60*(alpha*180/pi-degtran_1[0]);degtran_1[2]=(int) 3600*(alpha*180/pi-degtran_1[0]-degtran_1[1]);
    degtran_2[0]=(int) (delta*180/pi);degtran_2[1]=(int) 60*(delta*180/pi-degtran_2[0]);degtran_2[2]=(int) 3600*(delta*180/pi-degtran_2[0]-degtran_2[1]);
    printf("Terrestrial Time: %.3lf\nDistance: %.3lf km\nTheta: %d°%d\'%d\'\'\n",Exp_time,r/1000,degtran_0[0],degtran_0[1],degtran_0[2]);
    printf("Alpha: %d°%d\'%d\'\'\n",degtran_1[0],degtran_1[1],degtran_1[2]);
    printf("Delta: %d°%d\'%d\'\'\n",degtran_2[0],degtran_2[1],degtran_2[2]);
    return 0;
}
